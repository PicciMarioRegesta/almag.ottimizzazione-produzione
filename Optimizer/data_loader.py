...
class DataLoader:
    ...
    def load_data(self):
        ...
        famiglia_per_perc_materiale = composizioni_famiglia / 100
        
        # Caricamento dei valori minimi, massimi e medi dal foglio "Toll Fonderia Medio"
        # Carica la sezione "Minimo" (colonne C-O)
        ...
        # Carica la sezione "Massimo" (colonne P-AB)
        ...
        # Carica la sezione "Media" (colonne AC-AO)
        ...
        # Assicuriamoci che tutti i DataFrame abbiano gli stessi indici
        ...
        # Crea range_ricette_per_materiali con valori minimi, medi e massimi effettivi
        range_ricette_per_materiali = np.full((val_att.shape[0], val_att.shape[1]), None)
        for iy, ix in np.ndindex(val_att.shape):
            if val_att[iy, ix] == 0:
                range_ricette_per_materiali[iy, ix] = (None, None, None)
            else:
                min_val = val_min[iy, ix]/100
                att_val = val_att[iy, ix]/100
                max_val = val_max[iy, ix]/100
                range_ricette_per_materiali[iy, ix] = (min(min_val, max_val), att_val, max(min_val, max_val))
        ...
        return {
            ...
            'composizioni_famiglia': composizioni_famiglia
        }